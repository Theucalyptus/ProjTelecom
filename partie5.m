...
u(1) = 1;
k_s2 = kron(Dk_s2', u);
k_s2 = [k_s2, zeros(1, L/2*Ns_s2)];
h_8psk=filter(B, 1, k_s2); % signal bande de base
h_8psk=h_8psk(L/2*Ns_s2+1:end); % suppression des valeurs nulles à cause du retard du filtre
...
for j=1:length(EbN0)
    ...
    h_bruite_8psk = bruit_complexe(h_8psk, Ns, M, ebn0);
...
semilogy(EbN0_db, TEB_8psk, "r-*")
semilogy(EbN0_db, TEB_8psk_th, "b-*")
